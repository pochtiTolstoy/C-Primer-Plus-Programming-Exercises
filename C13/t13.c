...
void fill_digit_file(FILE** fp, int cols, int rows);
...
void generate_default(FILE** fp, int choice, int cols, int rows);
...
void menu(void) {
	printf("\n====================Меню===================\n");
	printf("1. Создать цифровое изображение [30x20 для паттернов].\n");
...
void create_picture_from_digital(void) {
	...
	*/
	...
	count_cols_n_rows(&fp_in, &cols, &rows);
	if (cols < 1 || cols > 256 || rows < 1 || rows > 256) {
...
void fill_digit_file(FILE** fp, int cols, int rows) {
	...
	switch (choice) {
		case 1:
			generate_default(fp, 1, cols, rows);
			break;
		case 2:
			generate_default(fp, 2, cols, rows);
			break;
		case 3:
			generate_wave(fp);
			break;

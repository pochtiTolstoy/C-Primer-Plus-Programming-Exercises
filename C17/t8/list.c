...
bool AddItemList(Item item, List* plist) { // Принимает Item, который нужно добавить и plist указатель на указатель на ноду
...
bool findItemList(const Item* item, List* plist) {
	Node* ptr = *plist;
	while (ptr != NULL) {
		if (strcmp(ptr->item.petkind, item->petkind) == 0) {
			return true;
		}
		ptr = ptr->next;
	}
...
void TraverseList(const List* plist, void (*pfun)(Item item)) { // принимает укзатель на указатель на node 
